 ...
 *
 * @author Vinayak, Sahil
 */

public class TransactionReductionUtil {
    final long OFFSET = 1000000000L;
    Set<Long> visitedEdges;

    public String createGraphForDebts(ArrayList<String> people, ArrayList<Expense> expenses) {

//        String[] person = { "Tanmay", "Shubham", "Vinayak", "Jangid", "Sahil", "Priyanshu", "Tarun"};
        
        //  List of all people in the group
        String[] person = new String[people.size()];
        int it = 0;
        for(String p: people) {
            person[it++] = p; // probable trouble point
        }
        int n = person.length;

        Log.d("kkg-3",  "" + people.size());
        Log.d("kkg-2",  "" + expenses.size());
        Log.d("kkg-2",  "" + people.size());
        Log.d("kkg-1", "createGraphForDebts: " + n + person.toString());

        //  Creating a graph with n vertices
        Dinics solver = new Dinics(n, person);

        //  Adding edges to the graph
        solver = addAllTransactions(solver, expenses);
...
//        System.out.println();
    ...
    Dinics addAllTransactions(Dinics solver, ArrayList<Expense> expenses) {
//        solver.addEdge(1, 2, 40);
//        solver.addEdge(0, 2, 80);
...
//        solver.addEdge(6, 3, 80);
        Log.d("kkg-expenses", expenses.toString());

        for(Expense expense: expenses) {
            int sizeOfTakers = expense.getExpenseTakerIds().size();
            long eachTransactionAmount = expense.getTransactionAmount()/sizeOfTakers;
            for(int takerId: expense.getExpenseTakerIds()) {
                solver.addEdge(expense.getExpenseGiverId(), takerId, eachTransactionAmount);
            }
        }
...
abstract class NetworkFlowSolverBase {
    ...
    protected  final long INF = Long.MAX_VALUE / 2;
    ...
    private void initializeGraph() {
        ...
        Log.d("kkg-h", "initializeGraph: " + n + graph);
    }
    ...
    public String printEdges() {
        String ans = "";
        for(Edge edge : edges) {
            ans += String.format("%s ----%s----> %s", vertexLabels[edge.from], edge.capacity, vertexLabels[edge.to]);
        }
        Log.d("kkg-loggraph", "printEdges: ");
        return ans;
    }
